function [uerr,duerr] = BurgersTestProbZeroSpError(TC,m,s,p,q,scheme_no,mu,tf) 
    ...
    for i = 1:nt
        ...
        for j = 1:s
            ...
            bc_ux = G(TC,xi,t(j),h); f_vec = F(xi,t(j));
            % Calculating the right hand side: U(t).*(N*U(t)+G(t)) + F(t)
            R(:,j) = g_j.*(N*g_j + bc_ux) + f_vec;
        end
        u = u + dt*(R*b');
        tn = tn+dt;
        if i==nt
...
%     % plot
%     plot(x,true_uvec,':k','LineWidth',2)
%     hold on
%     plot(x,num_uvec,'--r','LineWidth',2)
end
