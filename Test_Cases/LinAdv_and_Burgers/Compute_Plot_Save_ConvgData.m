...
switch eqn 
    ...
    case 'Burgers'
        ...
        Cof5 = [8e-6,8e-4,7e-0]; Sl5 = [2,3,4];lim5 = [1e-13,5e-5];
...
for i = 1:3
    ...
    hold on
    loglog(dts,dU_Err(i,:),'linestyle',linS{2},'color',C{i},'marker',Mar{i},'MarkerSize',ms)
    hold on
end
hold on
loglog(dts(st31:end31),Cof3(1)*dts(st31:end31).^Sl3(1),'--','color',Cref{1})
loglog(dts(st32:end32),Cof3(2)*dts(st32:end32).^Sl3(2),'-.','color',Cref{1})
loglog(dts(st33:end33),Cof3(3)*dts(st33:end33).^Sl3(3),'-','color',Cref{1})
...
set(gcf,'PaperPosition',[0 0 400/100 550/100])
for i = 4:6
    loglog(dts,U_Err(i,:),'linestyle',linS{1},'color',C{i-3},'marker',Mar{i-3},'MarkerSize',ms)
    hold on
    loglog(dts,dU_Err(i,:),'linestyle',linS{2},'color',C{i-3},'marker',Mar{i-3},'MarkerSize',ms)
    hold on
end
hold on
loglog(dts(st41:end41),Cof4(1)*dts(st41:end41).^Sl4(1),'--','color',Cref{1})
loglog(dts(st42:end42),Cof4(2)*dts(st42:end42).^Sl4(2),'-.','color',Cref{1})
loglog(dts(st43:end43),Cof4(3)*dts(st43:end43).^Sl4(3),'-','color',Cref{1})
...
set(gcf,'PaperPosition',[0 0 400/100 550/100])
for i = 7:9
    loglog(dts,U_Err(i,:),'linestyle',linS{1},'color',C{i-6},'marker',Mar{i-6},'MarkerSize',ms)
    hold on
    loglog(dts,dU_Err(i,:),'linestyle',linS{2},'color',C{i-6},'marker',Mar{i-6},'MarkerSize',ms)
    hold on
end
hold on
loglog(dts(st51:end51),Cof5(1)*dts(st51:end51).^Sl5(1),'--','color',Cref{1})
loglog(dts(st52:end52),Cof5(2)*dts(st52:end52).^Sl5(2),'-.','color',Cref{1})
loglog(dts(st53:end53),Cof5(3)*dts(st53:end53).^Sl5(3),'-','color',Cref{1})
legend('','Location', 'northoutside')
switch eqn
    case 'LinAdv'
        legend({'(7,5,1):u','(7,5,1):u_x','(9,5,4):u','(9,5,4):u_x',...
        '(10,5,5):u','(10,5,5):u_x',sprintf('Slope %.1f',Sl5(1)),...
        sprintf('Slope %d',Sl5(2)),sprintf('Slope %d',Sl5(3))},'NumColumns',3)
    case 'Burgers'
        legend({'(7,5,1):u','(7,5,1):u_x','(9,5,4):u','(9,5,4):u_x',...
        '(10,5,5):u','(10,5,5):u_x',sprintf('Slope %d',Sl5(1)),...
        sprintf('Slope %d',Sl5(2)),sprintf('Slope %d',Sl5(3))},'NumColumns',3)
end
xlim([dts(end),dts(1)])
ylim([lim5(1),lim5(2)])
